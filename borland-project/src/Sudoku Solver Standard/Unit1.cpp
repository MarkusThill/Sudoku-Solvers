//---------------------------------------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------------------------
/*-----------------------------SUDOKU SOLVER 1.0---EIN PROJEKT VON MARKUS THILL----------------------------------------------*/
#include <vcl.h>
#pragma hdrstop
#include "Unit1.h"
//---------------------------------------------------------------------------------------------------------------------------
...
__fastcall TForm1::TForm1(TComponent* Owner)
	: TForm(Owner)
{
	/*Im Konstruktor werden alle Elemente des Arrays feld[x][y], feldtemp[x][y] auf 0 gesetzt,
...
void __fastcall TForm1::FormCreate(TObject *Sender)
{
    ...
    Die Panels und die Editfelder werden in Form von Arrays zur Laufzeit erstellt und es
    werden einige Eigenschaften bereits eingestelllt, wobei die Edit-Felder zweidimensional sind (Edit[x][y]).
    Zu beachten ist, dass die Edit-Felder eine abgeleitete Klasse(TEditCenter) von TEdit sind.*/
    ...
    for(i=0;i<3;i++)
    {
    	for(j=0;j<3;j++)
        {
    		Panel[k]= new TPanel(Form1); //Panel zur Laufzeot generieren
        	Panel[k]->Left = abstand_links_panel + (i*panel_breite)+(i);//Abstand zum linken Rand des Forms
        	Panel[k]->Top = abstand_panel_oben + (j*panel_hoehe)+(j);  //Abstand zum obereb Rand des Forms
    ...
    for(j=0;j<9;j++)
    {
    	for(i=0;i<9;i++)
        {
        	...
        	Edit[i][j] = new TEditCenter(Panel[panel_nr]); //Edit-Felder zur Laufzeit generieren
...
void __fastcall TForm1::Neu1Click(TObject *Sender)
{
    ...
    Messagebox überprüft. Falls der User bestätigt, werden alle EditFelder gelöscht und die Pfade
    der Save- und Open-Dialoge werden zurückgesetzt*/
...
Boolean TForm1::EditPruefen(void)
{
    ...
    for(i=0;i<9;i++)
    	for(j=0;j<9;j++)
        	if(Edit[i][j]->Text!="")//Überprüfen, ob das Feld gefüllt ist, da ansonsten eine Zugriffsverletzung auftreten würde
            {
            	count++;
            	if(Edit[i][j]->Text[1]<49 || Edit[i][j]->Text[1]>57)//Ob Inhalt des Feldes 1-9 ist
                	ret=false;
            }
    if(count<17)
    	ret=false;
	...
	for(i=0;i<9;i++)
		for(k=1;k<10;k++)
		{
			...
			if(count>1)
            {
            	for(p=0;p<18;p+=2)
                	if(temp[p]>=0)
                    	Edit[temp[p]][temp[p+1]]->Font->Color=clRed;//Wenn Ziffern öfters vorkommen, Farbe der bestroffenen Edits ändern
                    else
                    	break;
				return false;
            }
...
{
	/*Die Farbe der Schrift von allen Edit-Feldern wird auf Schwarz gesetzt*/
...
void TForm1::ZeitmessungStop(void)
{
    ...
    Label2->Caption= "Zeit: "+(AnsiString)(sec*1000+msec)+" MSec";
}
...
Boolean TForm1::HauptBerechner(void)
{
    ...
    analytisch zu lösen. Klappt dies nicht, dann wird eine Ziffer für ein
    Feld geraten und dann wieder versucht das Sudoku analytisch zu lösen. Klappt
    dies auch nicht, dann wird für das Feld eine andere Ziffer geraten usw.*/
    ...
    do
    {
        ...
        if(FeldVoll()) //Wenn alle Felder besetzt sind
        {
        	if(LoesungPruefen()) //Stimmt denn die Loesung??
            	beenden=true;  // Wenn ja, dann kann die Methode beendet werden und in die nächst höhere Ebene zurückgekerht werden
            else
            	return false; //Wenn nicht, dann wird ein false an die höhere Ebene zurückgegeben
        }
        if(!gemacht) //Wenn das Sudoku analytisch nicht mehr weiter lösbar ist
        {
            Boolean weitermachen=true;
            for(min=1;min<9;min++) //Den Mindestwert der Anzahl der Moeglichkeiten
            	for(i=0;i<9;i++)
            		for(j=0;j<9;j++)
                    {
                		if(feld[j][i]==0) //Wenn das Feld leer ist
                        {
                        	anzahl=MoeglichkeitenAnzahl(j,i); //Moeglichkeiten für das Feld zählen
                            if(anzahl==0) //Wenn die Anzahl 0 ist, dann stimmt etwas nicht, und es wird zur höheren Ebene zurückgekehrt
                            	return false;
                    		if(anzahl==min) //Stimmt die Anzahl der Möglichkeiten mit dem Mindestwert überein
                            	for(k=0;k<9;k++) //Alle Möglichkeiten durchgehen
                                	if(moeglichkeiten[j][i][k]>0)
                                    {
                                    	weitermachen=false;
                                        for(y=0;y<9;y++) //Array feld[][] temporär speichern
                                        	for(x=0;x<9;x++)
                                            	tempfeld[x][y]=feld[x][y];
                                    	feld[j][i]=moeglichkeiten[j][i][k];
                                        if(HauptBerechner()) //Es wird eine Ebene tiefer gegangen und dort versucht das Sudoku zu lösen
                                        	return true;
                                        for(y=0;y<9;y++)//temporäres feld wieder ins array feld[][] speichern
                                        	for(x=0;x<9;x++)
